...
/*
    * Each parser function takes a bool argument, called get, indicating whether the
    function needs to call Token_stream::get() to get the next Token.

    * Each parser function evaluates its expression and returns the value.
    * The function expr() handles addition and subtraction. 
    * Consists of a single loop that looks for terms to add or subtract:
*/
...
/*
    * The function term() handles multiplication and division in the same way expr() handles
...
/*
    * prim() handles a primary just likst expr() and term(), except that because we are getting
    lower in the call hierarchy a bit of real work is being done and no loop is necessary:
*/
double expr(double);
...
/*
    * When a Token that is a number(that is, an int or float literal) is seen,
        its value is palced in its number_value.
    
    * When a Token that is a name (however defined;) is seen, its value is placed in its string_value.


    NOTE: prim() always reads one more Token than it uses to analyze its primary expression.
*/
